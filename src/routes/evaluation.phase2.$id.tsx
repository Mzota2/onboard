...
export const Route = createFileRoute("/evaluation/phase2/$id")({
  beforeLoad: requireAuth,
  head: ({ params }) => ({
    meta: [{ title: `Phase 2 Evaluation · ${params.id} · onboard` }],
  }),
  component: Phase2EvalPage,
});

function Phase2EvalPage() {
  ...
  const { data: evaluations = [] } = useQuery({
    queryKey: ["evaluations", id, "phase2"],
    queryFn: () => listEvaluations(id, "phase2"),
    enabled: !!id,
  });

  const { data: aggregated } = useQuery({
    queryKey: ["aggregated-scores", id, "phase2"],
    queryFn: () => aggregateCandidateScores(id, "phase2"),
    enabled: !!id,
  });
  ...
  return (
    <AppShell>
      <div className="bp-fade-up">
        <p className="bp-meta">Phase 2 › Live Interview Evaluation</p>
      ...
      <article className="bp-card-shadow mt-5 p-5 text-center">
        ...
        <p className="bp-label mt-4">Phase 2 Aggregate Score</p>
        <p className="font-display text-5xl font-extrabold">{averageScore > 0 ? `${averageScore}%` : "—"}</p>
      ...
      <ResultExportActions
        ...
        phaseScores={{ phase2: averageScore }}
      />

      <div className="mt-5 grid grid-cols-2 gap-3">
        <Link to="/phase2" search={{ candidateId: candidate.id }} className="border-2 border-ink py-4 text-center font-mono text-[11px] uppercase tracking-widest bp-press">
          Back to Phase 2
        </Link>
